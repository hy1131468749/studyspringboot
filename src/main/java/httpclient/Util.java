...
public class Util
{
	 ...
	 * @return
	 */
	public static boolean checkSignature(ReceiverDataUpload data, String token)
	{
		// 计算接受到的消息的摘要
		// token长度 + 8B随机字符串长度 + 消息长度
		byte[] signature = new byte[token.length() + 8 + data.getMsg().toString().length()];
		System.arraycopy(token.getBytes(), 0, signature, 0, token.length());
		System.arraycopy(data.getNonce().getBytes(), 0, signature, token.length(), 8);
		System.arraycopy(data.getMsg().toString().getBytes(), 0, signature, token.length() + 8,
				data.getMsg().toString().length());
		...
		logger.info("check signature: result:{}  receive sig:{},calculate sig: {}",
				calSig.equals(data.getMsgSignature()), data.getMsgSignature(), calSig);
		return calSig.equals(data.getMsgSignature());
	}
	 ...
	 */
	public static String decryptMsg(ReceiverDataUpload data, String encodeKey)
			throws NoSuchPaddingException, NoSuchAlgorithmException, InvalidAlgorithmParameterException,
			InvalidKeyException, BadPaddingException, IllegalBlockSizeException
	{
		byte[] encMsg = Base64.decodeBase64(data.getMsg().toString());
	...
	public static BodyObj resolveBody(String body, boolean encrypted)
	{
		...
		try
		{
			ObjectNode jsonMsg = JsonUtil.string2JsonObj(body);

			obj = new BodyObj();
			obj.setNonce(jsonMsg.get("nonce").asText());
			obj.setMsgSignature(jsonMsg.get("msg_signature").asText());
			if (encrypted)
			{
